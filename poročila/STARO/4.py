...
# 10-dnevno pravilo po predmetu (po imenu, simetrično – gleda naprej in nazaj)
rows_10dni = []
problem_10_pairs = set()  # (Predmet, Datum)
df_with_dates = df_norm[df_norm["Datum"].notna()]

for predmet, grp in df_with_dates.groupby("Predmet"):
    ...
    for i in range(len(dates) - 1):
        ...
        if delta < 10:
            ...
            problem_10_pairs.add((predmet, d1))
            problem_10_pairs.add((predmet, d2))

df_10dni = pd.DataFrame(rows_10dni)
df_norm["is_10dni"] = df_norm.apply(
    lambda r: (r["Predmet"], r["Datum"]) in problem_10_pairs,
    axis=1
)
...
conflict_records = []
seen_pairs = set()  # da ne podvajamo istih parov
...
# =============================
# 4. PREDLOGI NOVIH DATUMOV (TOP 5)
# =============================
...
def propose_dates_for_row(row, df_all: pd.DataFrame, top_n: int = 5):
    ...
    - brez vikendov,
    - brez praznikov,
    - brez 10-dnevnega konflikta (naprej in nazaj) z drugimi roki istega predmeta,
    ...
    # druge datume istega predmeta (ostali roki)
    mask_same_predmet = (df_all["Predmet"] == predmet) & df_all["Datum"].notna()
    # izločimo to vrstico po indexu
    other_dates = df_all[mask_same_predmet & (df_all.index != row.name)]["Datum"].tolist()

    # vse druge izpite v istem roku po datumih
    ...
    while current <= end:
        ...
        if orig_date is not None and current == orig_date:
        ...
        # 10-dnevno pravilo – gleda naprej in nazaj (simetrično)
        conflict_10 = False
        ...
        # prekrivanje obveznih – samo če je predmet sploh v kakšni skupini
        if predmet_groups:
        ...
        # če pridemo do sem, je kandidat OK
        dist = None
    ...
    # sortiraj po oddaljenosti, nato po datumu
    candidates_sorted = sorted(candidates, key=lambda x: (x[1], x[0]))

    # vrni samo top_n datumov
    return [c[0] for c in candidates_sorted[:top_n]]


# določimo, katere vrstice so "problematične"
df_norm["is_problem"] = (
    df_norm["is_vikend"] |
    df_norm["is_izven"] |
    df_norm["is_praznik"] |
...
# =============================
# 5. ZAPIS POROČILA Z DINAMIČNIM IMENOM
# =============================
