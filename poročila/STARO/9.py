...
# =============================
# FUNKCIJA: GENERIRANJE KOLEDARJA OBVEZNIH PREDMETOV
# =============================

def generiraj_koledar(df_norm):
    """
    Iz df_norm generira izpitni koledar obveznih predmetov po rokih,
    barve po konfliktnih skupinah, pri predmetih v več skupinah pa
    dodatno obarvana sosednja celica v vrstici.
    """
    # barvna paleta za skupine (ARGB)
    COLORS = [
        "00FFE6CC",
        "00FFF2CC",
        "00D0E0E3",
    ...
    for rok, skupine in KONFLIKTNE_SKUPINE_PO_ROKU.items():
        ...
        for idx, grp in enumerate(skupine):
            ...
            group_colors[(rok, idx)] = PatternFill(start_color=color, end_color=color, fill_type="solid")

        all_obvezni.update(s)

    # za jesensko – vzemi vse obvezne, a brez posebnih skupin (brez barv po skupinah)
    if "jesensko" not in obvezni_predmeti:
        obvezni_predmeti["jesensko"] = all_obvezni
        predmet_groups["jesensko"] = defaultdict(list)

    wb = Workbook()
    # odstrani default sheet
    ...
    for rok, sheetname in [("zimsko", "zimsko"), ("poletno", "poletno"), ("jesensko", "jesensko")]:
        ...
        # header: Predmet + datumi
        ws.cell(row=1, column=1).value = "Predmet"
        dates = []
        cur = start
        col_idx = 2
        while cur <= end:
            ws.cell(row=1, column=col_idx).value = cur
            dates.append(cur)
            col_idx += 1
        ...
        # filter, freeze, wrap, width za ta sheet
    ...
    base_name = "izpitni_koledar_OBVEZNI_auto"
    ext = ".xlsx"
...
# =============================
# 8. GENERIRAJ KOLEDAR OBVEZNIH
# =============================

generiraj_koledar(df_norm)
